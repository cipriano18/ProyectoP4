...
useEffect(() => {
    const cargarExpositores = async () => {
        ...
        const adaptados = data.map((ex: any) => ({
            ...
            cursosSeleccionados: ex.cursos?.map((c: any) => c.curso_id) || [],
        }));
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Recargar lista de expositores
      const actualizados = await fetch("/api/exhibitors");
      ...
      alert("No se pudo registrar el expositor.");
    }
  };

  const iniciarEdicion = (expositor: Expositor) => {
    // Asegurar que cursosSeleccionados esté definido
    setForm({
      ...expositor,
  ...
  const eliminarExpositor = (id: number) => {
    setExpositores((prev) => prev.filter((ex) => ex.id !== id));
    if (editandoId === id) {
      setEditandoId(null);
      setForm({});
    }
                ...
                <div className="glassmorphism p-6 rounded-3xl border border-white/30">
                    ...
                    <div className="mt-6 max-w-xs space-y-4">
                        ...
                        <div>
                            ...
                            {cursos.length === 0 && <p className="text-sm italic text-white/70">No hay cursos disponibles.</p>}
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => {
                                            ...
                                            setForm((prev) => {
                                                const actuales = prev.cursosSeleccionados || [];
                                                if (checked) {
                                                    return { ...prev, cursosSeleccionados: [...actuales, c.curso_id] };
                                                } else {
                                                    return {
                                                        ...prev,
                                                        cursosSeleccionados: actuales.filter((id) => id !== c.curso_id),
                                                    };
                                                }
                                            });
                                        }}
                            ...
                            {expositores
                                ...
                                .map((ex) => (
                                    ...
                                    >
                                        <div>
                                            <p>
                                                <strong>Nombre:</strong> {`${ex.p_nombre} ${ex.s_nombre} ${ex.p_apellido} ${ex.s_apellido}`}
                                            </p>
                                            <p>
                                                <strong>Cédula:</strong> {ex.ced_expo}
                                            </p>
                                            <p>
                                                <strong>Correo:</strong> {ex.correo_elect}
                                            </p>
                                            <p>
                                                <strong>Especialidad:</strong> {ex.especialidad}
                                            </p>
                                        </div>
